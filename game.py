...
timer = Label( time_value, 350, 50, fill='dimGrey', size=30, font='orbitron')
...
def gameOver():
    ...
    #Label('time: ' + str(time_value) + ' seconds', 200, 200, size=20, fill='White')
    app.stop()
...
def makeBlock():
    ...
    y = (-1) * 15
    newBlock = Group(
        Image('cmu://1204779/46835549/6061515b-be14-4c56-ac54-b6c7c9ba16ba.png',x + 5, y, width=25, height=45, opacity=55, fill='red')
    )
    blocks.add(newBlock)
...
def onStep():
    ...
    #Boss 1 (create a pause value to pause the music at 150, everytime i do this i break the game somehow... lol)
    if (time_value >=10):
      if (time_value == 10):
          music.play(loop=False)
      elif (time_value >=70):
        music.play(loop=False)
    if (time_value >= 5 and time_value <= 10):
        bossWarning.visible = True
    if (time_value >= 10 and time_value <= 40):
        bossWarning.visible = False
        Boss.centerY -= 1.25
        if (Boss.centerY <= 400):
            Boss.centerY = 400
        if (Boss.centerY <= 400 and time_value <= 90):
            Hand.centerY = 325
        if (time_value <= 90 and time_value >= 10):
            shield.visble = True
            LaserL.centerX -= 4
            LaserR.centerX += 4
            if (app.steps % 100 == 0):
            ...
            if (app.steps % 100 == 0):
                ...
                if (rand == 1):
                    makeLaserR()
    if (time_value >= 105 and time_value <= 120):
        shield.visible = False
        Boss.centerY += 2
        if (Boss.centerY == 800):
            Boss.centerY = 800
            
    if (time_value >= 99 and time_value <= 100):
        Hand.centerY -= 3
    if (time_value >= 100):
        Hand.centerY += 6
    ...
    if (time_value >= 120):
        meteor.centerY += 6
    if (time_value >= 120 and time_value <= 150):
        if (app.steps % 30 == 0):
            rand = randrange(0, 2)
            if (rand == 1):
                makeBoss()
    ...
    if (time_value >= 180):
        ...
        if (Boss2.centerX <= 325):
            Boss2.centerX = 325
        if (time_value >= 180 and time_value <= 240):
            if (app.steps % 110 == 0):
                rand = randrange(0, 2)
                if (rand == 1):
                    makeMissile()
    ...
    if (time_value >= 270 and time_value <= 285):
        ...
        player.centerX += 0.75
    if (time_value >= 285 and time_value <= 300):
        conveyer1.centerX -= 0.75
        conveyer2.centerX -= 0.75
    ...
    app.steps += 1
    time_value = int(lava.top - player.bottom - 45 + 175) #int(lava.top - player.bottom - 45)
